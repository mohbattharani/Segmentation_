...
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 54
...
def get_filename_list(path):
  ...
  for i in fd:
    i = i.strip().split(" ")
    image_filenames.append(dataset_path+i[0])
    label_filenames.append(dataset_path+i[1])
...
def CamVidInputs01(path_images, path_labels, batch_size):

  #print ('Path Images:', path_images)
  #print ('Path labels:', path_labels)
  image_filenames = []
  label_filenames = []
  file_images = open(path_images)
  ...
  images, label_batch = tf.train.batch([reshaped_image, label],batch_size=1,num_threads=1,
           capacity=21)
...
def get_all_test_data(im_list, la_list):
  ...
  for im_filename, la_filename in zip(im_list, la_list):
    ...
    if (index == 0):
        img1 = scp.misc.imread(im_filename)
        scp.misc.imsave('output/Template.png', img1)
        index = index + 1
...
def get_all_images(im_list):
  images = []

  for im_filename, la_filename in zip(im_list, im_list):
    #print ('im_filename:', im_filename)
    im = np.array(skimage.io.imread(im_filename), np.float32)
    im = im[np.newaxis]
    images.append(im)
